...
export class SearchWaterBehavior extends AIBehavior {
  ...
  constructor() {
    super(BehaviorPriority.MEDIUM, 'Searching Water');
  }
  ...
  shouldExecute(npc: NPCEntity): boolean {
    ...
    if (npc.getWorldMap().isNextToWater(npc.getPosition())) return false;
